# build.py — supermogu/ 모듈들을 단일 배포 html로 병합
# 산출물: ../supermogu.html (더블클릭 실행, 외부 의존 없음)
import base64
import re
from pathlib import Path

HERE = Path(__file__).parent
OUT = HERE.parent / "supermogu.html"

html = (HERE / "index.html").read_text(encoding="utf-8")

# 1) 에셋 → base64 데이터 URI (슈퍼모구 전용 누끼 이미지)
def data_uri(p):
    b = (HERE.parent / p).read_bytes()
    return "data:image/png;base64," + base64.b64encode(b).decode()

assets_js = 'window.SMG = { ASSETS: { mogu: "%s" } };' % data_uri("supermogu/assets/mogu-super.png")
html = re.sub(
    r'<script>window\.SMG = \{ ASSETS: .*?\};</script>',
