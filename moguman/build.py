# build.py — moguman/ 모듈들을 단일 배포 html로 병합
# 산출물: ../moguman.html (더블클릭 실행, 외부 의존 없음)
import base64
import re
from pathlib import Path

HERE = Path(__file__).parent
OUT = HERE.parent / "moguman.html"
...
assets_js = 'window.MGM = { ASSETS: { mogu: "%s" } };' % data_uri("game/assets/mogu-icon.png")
html = re.sub(
    r'<script>window\.MGM = \{ ASSETS: .*?\};</script>',
