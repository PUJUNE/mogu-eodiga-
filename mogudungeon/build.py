# build.py — mogudungeon/ 모듈들을 단일 배포 html로 병합
# 산출물: ../mogudungeon.html (더블클릭 실행, 외부 의존 없음)
import base64
import re
from pathlib import Path

HERE = Path(__file__).parent
OUT = HERE.parent / "mogudungeon.html"

html = (HERE / "index.html").read_text(encoding="utf-8")

# 1) 에셋 → base64 데이터 URI (모구 얼굴 사진은 game/assets 재사용)
def data_uri(p):
    b = (HERE.parent / p).read_bytes()
    return "data:image/png;base64," + base64.b64encode(b).decode()

assets_js = 'window.MDN = { ASSETS: { mogu: "%s" } };' % data_uri("game/assets/mogu-icon.png")
html = re.sub(
    r'<script>window\.MDN = \{ ASSETS: .*?\};</script>',
