...
const shots = join(root, 'mogudragon', 'test');
...
check('런처 → 모구드래곤 타이틀', await page.evaluate(() =>
  !!window.MDG && !document.getElementById('title-screen').classList.contains('hidden')));
await page.screenshot({ path: join(shots, 'shot-title.png') });

// ══ 2. 키보드: 시작 → 이동 → 공격 → 처치 ══
await page.keyboard.press('Enter');
await page.waitForTimeout(500);
check('Enter → MISSION 1 시작', await page.evaluate(() => { const d = window.MDG._dbg(); return d.mode === 'play' && d.mission === 1 && d.enemies > 0; }));
await page.evaluate(() => {   // 악당 간섭 제거: 격리 + 플레이어 상태 리셋·무적
  const s = window.MDG._st();
  s.p.iv = 999; s.p.state = 'idle'; s.p.stT = 1;
  for (const e of s.enemies) { e.spd = 0; e.atkCd = 999; e.baseAtkCd = 999; e.x = -400; }
});
const p0 = await page.evaluate(() => window.MDG._dbg().p);
await page.keyboard.down('ArrowRight');
await page.keyboard.down('ArrowDown');
await page.waitForTimeout(300);
await page.keyboard.up('ArrowRight');
await page.keyboard.up('ArrowDown');
const p1 = await page.evaluate(() => window.MDG._dbg().p);
...
const d2 = await page.evaluate(() => window.MDG._dbg());
...
await page.evaluate(() => {
  ...
  s.p.x = window.MDG.Logic.sec(s).x0 + 60;
});
await page.waitForTimeout(600);
const d3 = await page.evaluate(() => window.MDG._dbg());
...
check('보스 격파 → 미션 클리어 화면', await page.evaluate(() =>
  window.MDG._dbg().mode === 'win' && !document.getElementById('win-screen').classList.contains('hidden')));
await page.screenshot({ path: join(shots, 'shot-win.png') });
await page.click('#btn-next');
await page.waitForTimeout(400);
check('다음 미션 → M2', await page.evaluate(() => window.MDG._dbg().mission === 2));
...
await mp.goto('http://localhost:8749/mogudragon.html');
...
check('처음부터 탭 → 플레이 + 패드 표시', await mp.evaluate(() =>
  window.MDG._dbg().mode === 'play' && getComputedStyle(document.getElementById('vpad')).display === 'block'));
await mp.evaluate(() => {
  const s = window.MDG._st();
  s.p.iv = 999; s.p.state = 'idle'; s.p.stT = 1;
  for (const e of s.enemies) { e.spd = 0; e.atkCd = 999; e.baseAtkCd = 999; e.x = -400; }
...
const tp0 = await mp.evaluate(() => window.MDG._dbg().p);
await press('vbtn-right', true);
await mp.waitForTimeout(300);
await press('vbtn-right', false);
const tp1 = await mp.evaluate(() => window.MDG._dbg().p);
...
check('👊 탭 → 처치', await mp.evaluate(() => window.MDG._dbg().score >= 100));
