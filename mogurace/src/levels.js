...
M.START_TIME = [23, 19, 20.5, 20.5, 21.5, 21.5, 21, 21, 18.5, 18, 24.5, 18, 22, 18, 19.5,
  21, 19.5, 17, 22, 23, 17, 16.5, 21, 19.5, 18, 25.5, 18, 21, 17, 19.5];
M.CP_BONUS = [17.5, 20.5, 19, 19.5, 19.5, 18, 17.5, 19, 18, 19, 22, 19.5, 21.5, 23.5, 19,
  22, 20, 23, 21.5, 19, 20, 23.5, 19.5, 17.5, 20, 20, 22.5, 20, 22.5, 19];
...
M.makeStage = function (no) {
  ...
  const segsTarget = Math.round(3750 + (no - 1) * 112.5);       // 코스 길이 (세그먼트)
  ...
  const trafficN = Math.round((15 + (no - 1) * 1.125) * (boss ? 1.35 : 1));
