# build.py — mogunamgeuk/ 모듈들을 단일 배포 html로 병합
# 산출물: ../mogunamgeuk.html (더블클릭 실행, 외부 의존 없음)
import base64
import re
from pathlib import Path

HERE = Path(__file__).parent
OUT = HERE.parent / "mogunamgeuk.html"
...
assets_js = 'window.MNG = { ASSETS: { mogu: "%s" } };' % data_uri("mogunamgeuk/assets/mogu-ski.png")
html = re.sub(
    r'<script>window\.MNG = \{ ASSETS: .*?\};</script>',
