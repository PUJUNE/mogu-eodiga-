...
M.makeStage = function (no) {
  ...
  const popP = Math.min(0.9, (0.4 + t * 0.35) * (D.popMul || 1));    // 크레바스 바다사자(숨었다 솟아오름) 출현률
  d = 300;
  while (d < length - 300) {
    const r = rng.next();
    if (r < 0.34) {
...
M.curveAt = function (stage, d) {
  ...
  const raw = (x) => {
    for (const cv of cs) if (x >= cv.d0 && x < cv.d1) return cv.c;
    return 0;
  };
  let b = cs[0].d0;                                  // 가장 가까운 경계
  for (const cv of cs) {
    if (Math.abs(cv.d0 - d) < Math.abs(b - d)) b = cv.d0;
    if (Math.abs(cv.d1 - d) < Math.abs(b - d)) b = cv.d1;
  }
  const h = CURVE_RAMP / 2;
  if (Math.abs(d - b) >= h) return raw(d);
  const p = (d - (b - h)) / CURVE_RAMP;
  const s = p * p * (3 - 2 * p);
  const c0 = raw(b - h), c1 = raw(b + h);
  return c0 + (c1 - c0) * s;
};
