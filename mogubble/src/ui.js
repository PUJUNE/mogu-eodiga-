...
M.save = {
  KEY: 'mogubble-save-v1',
  data: { best: 1, hiscore: 0 },
  load() {
  ...
  reach(no) { if (no > this.data.best) { this.data.best = Math.min(30, no); this.store(); } },
...
M.ui = {
  ...
  refreshTitle() {
    ...
    $('btn-continue').textContent = `이어하기 — ROUND ${best}`;
  ...
  hud(st) {
    ...
    $('hud-round').textContent = `ROUND ${st.no} · ${st.stage.theme.name}`;
  },
