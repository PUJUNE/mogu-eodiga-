...
M.Render = {
  ...
  drawW(st, w, team, wx, wz, T, t, rest) {
    ...
    if (down) {                                              // 다운: 대자 눕기
      ...
      B(-13, ly + 1, 3, 2, '#282838'); B(-13, ly + 5, 0, 0, '#282838');
      if (w.state === 'ko') {
    ...
    // 다리 + 부츠
    B(-3 + step, y + 28, 2, 6, skin); B(1 - step, y + 28, 2, 6, skinS);
    B(-3.5 + step, y + 38, 3, 2, '#282838'); B(0.5 - step, y + 38, 3, 2, '#282838');
    // 트렁크
    B(-4, y + 22, 8, 3.5, trunk);
    // 상체 (역삼각)
    B(-5, y + 10, 10, 6, skin);
    B(-5, y + 10, 2, 6, skinS);                              // 등 음영
    B(1.5, y + 14, 2, 1, skinS);                             // 가슴 라인
    // 팔: 공격 중 = 앞으로 길게 / 평시 = 리드팔 + 뒷팔
    if (w.state === 'atk') {
      B(4, y + 11, 6, 2, skin); B(9.5, y + 10.5, 2.5, 3, '#e8556a');
      B(-6.5, y + 12, 2, 4, skinS);
    } else if (w.state === 'air' || w.state === 'fba') {
      B(3, y + 8, 5, 2, skin); B(7.5, y + 7.5, 2, 2.5, '#e8556a');   // 위로 뻗은 팔
      B(-6, y + 12, 2, 3, skinS);
      B(-2 + step, y + 30, 3, 3, skinS);                     // 다리 접기 덧칠
    } else {
      B(4, y + 12, 3, 2, skin); B(6.5, y + 11, 2, 3, '#e8556a');     // 리드 글러브 (앞)
      B(-7, y + 12, 2, 5, skinS);
    }
    // 머리 (측면 프로필 — 방향 표현의 핵심)
