...
M.Logic = {
  ...
  step(st, dt, input) {
    ...
    if (this.canAct(P)) {
      ...
      if (dx && !(P.state === 'air' && P.dkVx)) P.face = dx; // 이동 방향으로 몸이 향함 (드롭킥 비행 중엔 고정)
      ...
      if (input.atk && P.cd <= 0 && P.state !== 'fba') {
        this._attack(st, P, E, 'p', ev, dx);                 // dx = 이동 의도 (드롭킥 방향 우선권)
        if (st.phase !== 'fight') return ev;
      }
  ...
  // 공격 분기 (지상: 라리아트/잡기/펀치, 공중: 드롭킥 돌진)
  // moveDir: 공격 순간 누르고 있던 좌우 이동 방향 (±1, 없으면 0/undefined)
  _attack(st, att, def, team, ev, moveDir) {
    const d = this.dist(att, def);
    const powered = att.poweredT > 0;

    if (att.state === 'air') {                               // 공중 공격 = 플라잉 드롭킥 돌진
      att.cd = 0.55;
      // 이동 중이면 이동 방향(의도 우선 — 교차 순간 역방향 발사 방지), 정지 점프는 상대 자동 조준
      att.face = moveDir || (def.x >= att.x ? 1 : -1);
      att.anim = 'dropkick';
