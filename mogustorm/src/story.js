...
(function () {
  ...
  NS.CHARS = {
    ...
    frances:  { name: "프랜시스",    icon: "🕊️", color: "#f0c8d8" },
    joseph:   { name: "조지프",      icon: "🐐",  color: "#b8b0a0" },
    lockwood: { name: "록우드",      icon: "🐇",  color: "#c8bca8" }
  };
  ...
  /* ════════════ 0부 — 액자: 록우드의 하룻밤 (원작 1~3장) ════════════ */

  S.p_1 = {
    bg: "heights_ext", weather: "snow", mood: "tense", cast: ["lockwood"],
    lines: [
      ["n", "1801년 겨울, 요크셔의 황야. 세상에서 가장 외진 셋집 '스러시크로스 그레인지'에 록우드라는 여행자 신사가 세를 들었습니다."],
      ["lockwood", "집주인에게 인사나 해 둘까. 워더링 하이츠 — '폭풍의 언덕'이라… 이름 한번 스산하군."],
      ["n", "언덕 저택의 주인은 벨벳 외투를 걸친 무뚝뚝한 잿빛 고양이 신사였습니다. 그날 밤 폭설에 갇힌 록우드는, 아무도 쓰지 않는 낡은 침실에서 하룻밤을 묵게 되지요."]
    ],
    next: "p_2"
  };
  S.p_2 = {
    bg: "night", weather: "storm", mood: "storm", cast: ["lockwood", "ghost"],
    lines: [
      ["n", "한밤중 — 덧창을 두드리는 소리에 록우드가 창을 연 순간, 얼음장 같은 작은 손이 그의 손목을 붙잡았습니다."],
      ["ghost", "들여보내 줘… 나, 캣서린이야. 집에 왔어. 황야에서 길을 잃었단 말이야… 벌써 20년이나…"],
      ["lockwood", "히이익—!! 노, 놓아라! 유령이다! 이 집엔 유령이 나온다고오!!"],
      ["n", "비명에 달려온 저택의 주인은, 록우드를 꾸짖는 대신 창을 활짝 열고 어둠에 대고 울부짖었습니다. — '들어와! 캐시, 제발, 한 번만 더!' …그 절규를 록우드는 평생 잊지 못했다고 합니다."]
    ],
    next: "p_3"
  };
  S.p_3 = {
    bg: "grange", weather: "none", mood: "sad", cast: ["lockwood", "nelly"],
    lines: [
      ["lockwood", "넬리 부인. 대체 그 언덕엔 무슨 사연이 있는 겁니까? 그 무서운 주인은 누구고, 캣서린은 또 누굽니까?"],
      ["nelly", "…긴 이야기랍니다, 손님. 벽난로에 장작을 더 넣지요. 모든 것은 40년 전 — 언쇼 영감님이 리버풀에서 고양이 한 마리를 데려온 밤부터 시작되었으니까요."]
    ],
    next: "a1_1"
  };
  ...
  S.a1_1 = {
    bg: "liverpool", weather: "rain", mood: "sad", cast: ["nelly", "earnshaw"],
    lines: [
      ["n", "리버풀 부두, 비 내리던 밤. 언쇼 영감님은 외투 속에 작고 꼬질꼬질한 떠돌이 고양이를 품은 채 60마일을 걸어 돌아오셨습니다."],
      ["earnshaw", "부둣가 상자 뒤에서 울고 있더구나. 이름은… 모구. 어려서 잃은 내 첫아이의 이름을 주마. 오늘부터 이 아이도 우리 식구다."]
    ],
    next: "a1_2"
  };
  S.a1_2 = {
  ...
  S.a1_6 = {
    bg: "heights_ext", weather: "cloud", mood: "tense", cast: ["hindley", "joseph"],
    lines: [
      ["n", "하지만 힌들리 도련님의 심술은 날로 심해졌습니다. 아버지의 사랑을 빼앗겼다고 여긴 거지요."],
      ["hindley", "이 떠돌이! 내 말을 훔쳐 탔지? 마구간 청소는 네 몫이다. 밥은 그 다음이야!"],
      ["joseph", "쯧쯧… 주워 온 것들은 은혜를 모르는 법이여. 일이나 하고, 저녁엔 설교나 들어라, 이놈아."]
    ],
    choices: [
  ...
  S.a1_8 = {
    bg: "hall", weather: "rain", mood: "sad", cast: ["hindley", "frances"],
    lines: [
      ["n", "세월이 흘러 힌들리 도련님은 도시의 대학으로 보내졌고 — 3년 뒤 가을, 언쇼 영감님은 벽난로 앞 의자에서 조용히 숨을 거두셨습니다."],
      ["n", "장례식 날, 힌들리 도련님이 돌아왔습니다. 낯선 신부 — 눈처럼 희고 잘 웃는 프랜시스 아씨의 손을 잡고서요."],
      ["hindley", "이제 이 집의 주인은 나다. 모구, 네 방은 오늘부터 헛간이야. 공부도 끝이다. 하인은 하인답게 살아라."],
      ["frances", "어머 여보, 저 아이가 그 얘기하던…? 후훗, 눈매가 사납기도 해라."],
  ...
  S.b1_frances = {
    bg: "hall", weather: "cloud", mood: "sad", cast: ["hindley", "nelly"],
    lines: [
      ["n", "이듬해 여름, 프랜시스 아씨는 아들 헤어턴을 낳고 — 반년을 넘기지 못한 채 폐병으로 세상을 떠났습니다."],
      ["hindley", "프랜시스… 프랜시스!! …술을 가져와. 전부 꺼져! 신도, 이 집도, 죄다 저주받으라지!!"],
      ["n", "그날부터 힌들리 도련님은 술과 도박에 자신을 내던졌고, 갓난 헤어턴은 내 품에서 자랐습니다. 어두워지는 저택에서 아가씨와 에드거 도련님의 혼담만이 촛불처럼 밝혀졌지요."]
    ],
    next: "b1_4"
  };
  S.b1_4 = {
  ...
  S.r_sick = {
    ...
    lines: [
      ["n", "봄이 오기 전 — 첫 아이를 품은 아가씨는 열병 끝에 눈에 띄게 야위어 갔습니다. 에드거 몰래, 나는 모구를 병실로 들였지요. …그게 마지막이 될 줄 알면서도요."],
      ["cat", "왔구나. …너와 에드거, 너희 둘이 내 심장을 부숴 놓고선, 둘 다 나더러 살라고 하지. 너무해, 정말."],
      ["cat", "모구. 내가 죽어도 황야에 있을게. 무덤 따위에 갇혀 줄 것 같아? 바람이 세게 부는 날엔… 그거, 나야."]
    ],
    choices: [
  ...
  S.sick_hate = {
    ...
    lines: [
      ["mogu", "왜 나를 버렸어! 네 심장을 배신한 건 너 자신이야, 캣서린! 너는… 너는 스스로를 죽인 거야!"],
      ["cat", "…그래. 맞아. 그러니까 용서하지 마. 대신… 잊지도 마. 그게 내 벌이고, 네 벌이야."],
      ["mogu", "그래도 가지 마. …유령이 되어도 좋아. 어떤 모습으로든 곁을 떠돌아. 나를 미치게 만들어도 좋으니 — 이 심연에 나 혼자만 두지 마!"],
      ["n", "그날 밤 아가씨는 딸 캐시를 낳고, 두 시간 뒤 조용히 숨을 거두었습니다. 창밖에는 때아닌 폭풍 — 그리고 모구의 귀에는, 그날부터 창을 두드리는 소리가 들리기 시작했다고 합니다."]
    ],
    next: "ghost_1"
  };
  S.ghost_1 = {
  ...
  S.sick_love = {
    ...
    lines: [
      ...
      ["n", "그날 밤 아가씨는 딸 캐시를 낳고 — 갓난 딸의 이마에 코를 대 본 뒤, 웃으며 눈을 감았습니다. 창밖에는 폭풍 대신, 그해 첫 히스꽃 바람이 불었지요."]
    ],
    next: "gen2_1"
  };
  S.sick_flee = {
  ...
  S.gen2_1 = {
    ...
    lines: [
      ["n", "아가씨가 떠나고 반년 뒤, 힌들리도 빈 술병 곁에서 생을 마쳤습니다. 저당 잡힌 저택은 문서째 모구의 것이 되었고 — 그 아들 헤어턴은, 옛날의 모구처럼 글자도 배우지 못한 하인으로 자랐지요."],
      ["n", "그리고 12년. 런던으로 달아났던 이사벨라가 죽으며 남긴 병약한 아들 린턴이 언덕으로 끌려왔습니다. 모구는 에드거가 병들어 죽어가는 사이 캐시 아가씨를 저택에 가두어 린턴과 강제로 혼인시켰고 — 그해 가을 린턴마저 시들듯 죽자, 그레인지의 문서까지 전부 모구의 손에 들어왔습니다."],
      ["mogu", "(저택 두 채, 두 가문의 핏줄 전부 내 발밑에. 언쇼가에게는 언쇼가가 내게 한 그대로. 린턴가에게는 린턴가가 내게 한 그대로. …복수의 판은 완성 직전이다.)"]
    ],
    next: "gen2_2"
  };
  S.gen2_2 = {
  ...
  S.end_ghost = {
    ...
    lines: [
      ["n", "그 겨울부터 모구는 곡기를 끊고 밤마다 황야를 헤맸습니다. 어디를 보아도 그 애가 보인다면서요. 그리고 어느 폭풍 치는 밤 — 창 두드리는 소리에, 모구는 처음으로 창을 활짝 열었습니다."],
      ["ghost", "…이제야 여는구나, 바보야. 황야에서 20년을 기다렸는데."],
      ["mogu", "미안. 문단속이 심한 집에서 자랐거든. …가자. 바위까지 경주야."],
      ["n", "이튿날 아침, 모구는 활짝 열린 창가에서 미소 띤 채 잠들어 깨지 않았습니다. 이듬해 새해 첫날, 헤어턴과 캐시는 부부가 되어 그레인지로 내려갔지요."],
      ["n", "훗날 록우드 씨는 나란한 세 개의 묘비 곁을 지나며 적었습니다 — '이 고요한 흙 속에 잠든 이들에게, 어찌 편치 않은 잠을 상상하랴.' …하지만 목동들은 지금도 말한답니다. 폭풍 치는 밤이면, 황야를 나란히 달리는 두 그림자를 본다고요."]
    ],
    ending: "ghost"
  };
  S.end_star = {
  ...
  S.end_avatar = {
    ...
    lines: [
      ["n", "모구는 흔들리지 않았습니다. 과부가 된 캐시 아가씨를 부엌데기로 부렸고, 헤어턴에게선 마지막 남은 것 — 캐시 곁에 머물 자격마저 빼앗았지요."],
