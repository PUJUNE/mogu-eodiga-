# build.py — mogufortress/ 모듈들을 단일 배포 html로 병합
# 산출물: ../mogufortress.html (더블클릭 실행, 외부 의존 없음)
import base64
import re
from pathlib import Path

HERE = Path(__file__).parent
OUT = HERE.parent / "mogufortress.html"
...
assets_js = 'window.MFT = { ASSETS: { mogu: "%s" } };' % data_uri("game/assets/mogu-icon.png")
html = re.sub(
    r'<script>window\.MFT = \{ ASSETS: .*?\};</script>',
