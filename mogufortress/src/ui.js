...
M.save = {
  ...
  record(no, stars) {
    ...
    if (no + 1 > this.data.best) this.data.best = Math.min(12, no + 1);
    this.store();
  },
};
...
M.ui = {
  ...
  showWin(st) {
    ...
    $('win-stats').innerHTML = `${st.stage.enemy.name} 격파! · 남은 HP ${st.p.hp}` +
      (st.stars === 3 ? '<br><b style="color:#7de08a">노데미지!!</b>' : '');
    $('btn-next').style.display = st.no < 12 ? '' : 'none';
