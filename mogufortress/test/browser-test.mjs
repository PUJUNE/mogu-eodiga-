...
const c0 = await page.evaluate(() => window.MFT._dbg().col);
await page.keyboard.down('ArrowRight');
await page.waitForTimeout(400);
await page.keyboard.up('ArrowRight');
const d0 = await page.evaluate(() => window.MFT._dbg());
check(`→ 홀드 → 이동 + 연료 소모 (col ${c0} → ${d0.col}, 연료 ${d0.fuel})`, d0.col > c0 + 2 && d0.fuel < 95);
...
const tc0 = await mp.evaluate(() => window.MFT._dbg().col);
await press('vbtn-left', true);
await mp.waitForTimeout(400);
await press('vbtn-left', false);
const tc1 = await mp.evaluate(() => window.MFT._dbg().col);
check(`◀ 홀드 → 이동 (col ${tc0} → ${tc1})`, tc1 < tc0 - 2);
